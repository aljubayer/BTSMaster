package com.OMCB.Model;

public class CSDAP {

    private String BSC = "";

    public String getBSC() {
        return BSC;
    }

    public void setBSC(String BSC) {
        this.BSC = BSC;
    }

    private String CSDAP = "";

    public String getCSDAP() {
        return CSDAP;
    }

    public void setCSDAP(String CSDAP) {
        this.CSDAP = CSDAP;
    }

    private String PLMN = "";

    public String getPLMN() {
        return PLMN;
    }

    public void setPLMN(String PLMN) {
        this.PLMN = PLMN;
    }

    private String bcfAbisIF = "";

    public String getBcfAbisIF() {
        return bcfAbisIF;
    }

    public void setBcfAbisIF(String bcfAbisIF) {
        this.bcfAbisIF = bcfAbisIF;
    }

    private String bcfTslShift = "";

    public String getBcfTslShift() {
        return bcfTslShift;
    }

    public void setBcfTslShift(String bcfTslShift) {
        this.bcfTslShift = bcfTslShift;
    }

    private String firstTSL = "";

    public String getFirstTSL() {
        return firstTSL;
    }

    public void setFirstTSL(String firstTSL) {
        this.firstTSL = firstTSL;
    }

    private String lastTSL = "";

    public String getLastTSL() {
        return lastTSL;
    }

    public void setLastTSL(String lastTSL) {
        this.lastTSL = lastTSL;
    }

    private String pcmCircuit_ID = "";

    public String getPcmCircuit_ID() {
        return pcmCircuit_ID;
    }

    public void setPcmCircuit_ID(String pcmCircuit_ID) {
        this.pcmCircuit_ID = pcmCircuit_ID;
    }
}
